...
<script>
    jQuery(document).ready(function () {
                jQuery('#{!!$uniqId!!} -map').locationpicker({
                    location: {
                        latitude: {
                !!$options['default_value']['lat']
                !!
            },
            longitude
    :
    {
        !!$options['default_value']['lng']
        !!
    }
    },
    radius: 0,
    ...
    onchanged: function (currentLocation, radius, isMarkerDropped) {
        ...
        jQuery('input[name="{!!$name.'[street]
        '!!}"]'
        ).
                val(addressComponents.addressLine1);
        jQuery('input[name="{!!$name.'[city]
        '!!}"]'
        ).
                val(addressComponents.city);
        jQuery('input[name="{!!$name.'[state]
        '!!}"]'
        ).
                val(addressComponents.stateOrProvince);
        jQuery('input[name="{!!$name.'[zip]
        '!!} "]'
        ).
                val(addressComponents.postalCode);
        jQuery('input[name="{!!$name.'[country]
        '!!}"]'
        ).
                val(addressComponents.country);
    }
    ,
    @endif
